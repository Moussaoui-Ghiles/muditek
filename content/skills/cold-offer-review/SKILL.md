...
- Use `FAIL` only for an evidenced contradiction, incoherence, or explicit mismatch.
- Use `UNPROVEN` when evidence is missing, inaccessible, stale for the claim, or insufficiently matched.
- Use `PASS` only for the bounded question tested; a pass never means the market will buy.
