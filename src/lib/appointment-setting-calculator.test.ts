...
describe("calculateAppointmentSettingQuote", () => {
  it("calculates held-meeting economics from buyer inputs", () => {
    ...
    expect(result?.costPerQualifiedHeldMeeting).toBeCloseTo(416.67, 2);
    expect(result?.expectedCac).toBeCloseTo(1666.67, 2);
    expect(result?.breakEvenCloseRate).toBeCloseTo(0.06944, 4);
    expect(result?.expectedGrossProfit).toBe(13000);
  });
